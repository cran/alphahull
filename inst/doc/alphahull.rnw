...
Finally, the function \code{areahull} returns the area of the {\mbox{$\alpha$-convex}} hull.  The idea behind this function is that, by joining the end points of adjacent arcs in the boundary of the {\mbox{$\alpha$-convex}} hull, we can define polygons that help us to determine the area of the estimator. 
...
The results in this document were obtained using \proglang{R} 2.10.0 (\cite{Rcran}). The \pkg{alphahull} package is available from the Comprehensive \proglang{R} Archive
Network at the url \url{http://CRAN.R-project.org/package=alphahull}. This
vignette describes version 0.2-0 of the package.
