...
ans<-.Fortran("delaunaycircum",
                  ...
                  as.integer(nt),
                  circenter=double(2 * nt),
                  lct=integer(tri.obj$nc),
                  tltri = integer(9 * nt),
                  ier=as.integer(0),
                 PACKAGE = "alphahull")
tri.info<-cbind(matrix(ans$tltri,nt,9,byrow=TRUE),matrix(ans$circenter,nt,2,byrow=TRUE))
colnames(tri.info)<- c("node1", "node2", "node3", "tr1", "tr2","tr3","arc1", "arc2","arc3","circumx", "circumy")
invisible(tri.info)
}
