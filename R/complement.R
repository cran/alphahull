...
# (mx1,my1),(mx2,my2) is the corresponding Voronoi segment
...
# For each Delaunay edge we determine if pm is between m1 and m2 (betw=1 if pm is between m1 and m2)
betw=numeric(length=n.edges)
vert<-mesh[,"mx1"]==mesh[,"mx2"]
if (sum(vert)>0){
betw[vert]<-apply(cbind(mesh[vert,"my1"],mesh[vert,"my2"],pm.y[vert]),1,rank)[3,]==2
}
if (sum(!vert)>0){
betw[!vert]<-apply(cbind(mesh[!vert,"mx1"],mesh[!vert,"mx2"],pm.x[!vert]),1,rank)[3,]==2
}
...
if (n.case1.1>0){
comp1.1<-cbind(mesh[case1.1,"mx1"],mesh[case1.1,"my1"],dm1[case1.1],matrix(mesh[case1.1,],nr=n.case1.1),1,v.x.m1[case1.1],v.y.m1[case1.1],theta.m1[case1.1])	
}

# By construction, bp1=0
comp1.2<-NULL
case1.2<-(mesh[,"bp1"]==0&dm1>alpha&aux>0&betw==1)|(mesh[,"bp1"]==1&aux>0&betw==1)
...
comp1.2<-cbind(e.x,e.y,alpha,matrix(mesh[case1.2,],nr=n.case1.2),1,v.x.m1[case1.2],v.y.m1[case1.2],theta)	
}

comp1.3.1<-NULL
case1.3<-(mesh[,"bp1"]==0&dm1>alpha&aux>0&betw==0)|(mesh[,"bp1"]==1&aux>0&betw==0)
...
if (n.case1.3.1>0){
comp1.3.1<-cbind(e.x[case1.3.1],e.y[case1.3.1],alpha,matrix(matrix(mesh[case1.3,],nr=n.case1.3)[case1.3.1,],nr=n.case1.3.1),1,v.x.m1[case1.3][case1.3.1],v.y.m1[case1.3][case1.3.1],theta[case1.3.1])
}
}
...
if (n.case2.1>0){
comp2.1<-cbind(mesh[case2.1,"mx2"],mesh[case2.1,"my2"],dm2[case2.1],matrix(mesh[case2.1,],nr=n.case2.1),2,v.x.m2[case2.1],v.y.m2[case2.1],theta.m2[case2.1])	
}

comp2.2<-NULL
case2.2<-(mesh[,"bp2"]==0&dm2>alpha&aux>0&betw==1)|(mesh[,"bp2"]==1&aux>0&betw==1)
...
comp2.2<-cbind(e.x,e.y,alpha,matrix(mesh[case2.2,],nr=n.case2.2),2,v.x.m2[case2.2],v.y.m2[case2.2],theta)	
}

comp2.3.1<-NULL
case2.3<-(mesh[,"bp2"]==0&dm2>alpha&aux>0&betw==0)|(mesh[,"bp2"]==1&aux>0&betw==0)
...
if (n.case2.3.1>0){
comp2.3.1<-cbind(e.x[case2.3.1],e.y[case2.3.1],alpha,matrix(matrix(mesh[case2.3,],nr=n.case2.3)[case2.3.1,],nr=n.case2.3.1),2,v.x.m2[case2.3][case2.3.1],v.y.m2[case2.3][case2.3.1],theta[case2.3.1])
}
}
...
if(mesh[index[i],"x1"]==mesh[index[i],"x2"]){
			a<-mesh[index[i],"x1"]
			if(mesh[index[i],"mx2"]<=a){sig=-4}
			else{sig=-3}
			comp.h[i,]=c(a,0,sig,mesh[index[i],],2,0,0,0)	
			
		}
		else{
			b<-(mesh[index[i],"y2"]-mesh[index[i],"y1"])/(mesh[index[i],"x2"]-mesh[index[i],"x1"])
			a<-mesh[index[i],"y1"]-mesh[index[i],"x1"]*b
			if(mesh[index[i],"my2"]<=a+b*mesh[index[i],"mx2"]){sig=-2}
			else{sig=-1}
			comp.h[i,]=c(a,b,sig,mesh[index[i],],2,0,0,0)	
