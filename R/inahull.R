...
n.ball<-length(ball)
in.compl<-FALSE
# We check if the point p=(p1,p2) belongs to a halfplane
	if (n.halfpl>=1){
		h<-1
...
# If the point does not belong to a halfspace, we check if it belong to any of the open balls definig the complement of the alpha-hull
	if (in.compl==FALSE){
		k<-1
